...
class LessonCRUDTestCase(TestCase):
    def setUp(self):
        ...
        self.owner_user = User.objects.create_user(username='owner', password='password', email='owner@example.com')
        self.moderator_user = User.objects.create_user(username='moderator', password='password', email='moderator@example.com')
        ...
        self.lesson_detail_url = reverse('study:lessons_retrieve',
                                         kwargs={'pk': self.lesson.id})  # /study/lessons/<id>/
        self.lesson_create_url = reverse('study:lessons_create')  # /study/lessons/create/
        self.lesson_update_url = reverse('study:lessons_update',
                                         kwargs={'pk': self.lesson.id})  # /study/lessons/<id>/update/
        self.lesson_delete_url = reverse('study:lessons_delete',
                                         kwargs={'pk': self.lesson.id})  # /study/lessons/<id>/delete/

        self.subscription_url = reverse('study:subscription_manage')  # Правильный URL через reverse
        self.client = APIClient()
    ...
    def test_lesson_list_owner(self):
        ...
        self.assertEqual(len(response.data['results']), 1)  # Проверяем пагинацию
    ...
    def test_lesson_list_unauthenticated(self):
        ...
        response = self.client.get(self.lesson_list_url)
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)  # Исправлено с 403 на 401
    ...
    def test_lesson_retrieve_other_user(self):
        ...
        self.client.force_authenticate(user=self.other_user)
        response = self.client.get(self.lesson_detail_url)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)  # Исправлено с 404 на 403
...
class SubscriptionTestCase(TestCase):
    ...
    def test_subscribe(self):
        ...
        self.assertTrue(Subscription.objects.filter(user_sub=self.user, course=self.course).exists())
